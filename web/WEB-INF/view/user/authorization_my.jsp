...
<body>
							...
							<tbody id="dataList">
								...
								<c:set var="account" value="${levels[l.authorizedGoods.id]}" />
									<tr data-row="${l.id}">
										...
										<td>
                                            <c:if test="${l.active}">
                                            <a class="btn btn-default viewBtn" target="letter" href="<c:url value='/auth/letter.html?id=${l.id}'/>" data-id="${l.id}">查看证书</a>
                                            </c:if>
                                        </td>
                                        <td>
                                        <c:if test="${l.active}">
                                        <a class="btn btn-default viewBtn"   onclick="level(${account.id},${l.id},${user.id})">点击生成</a>
                                        </c:if>
                                        </td>
										<%--<td><a class="btn btn-default viewBtn" target="letter" href="<c:url value='/auth/letterForm.html?uid=${user.id}'/>">点击生成</a></td>--%>
									</tr>
								</c:forEach>
							</tbody>
						</table>
							...
							<tbody>
										...
										<c:forEach items="${gacs}"
											...
											<tr>
												<td>${accounts.goods.name}</td>
												<td>${accounts.currentBalance}</td>
												<td>${accounts.currentCumulative}</td>
												<td>${accounts.cumulative}</td>
												<td>${accounts.tradingCumulative}</td>
                                                <td>${accounts.cumulativeRecord}</td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <select id="allLevelsSelect" class="form-control">
                        <c:forEach items="${allLevels}" var="al">
                        <option value="${al.id}">
                            ${al.name}
                        </option>
                        </c:forEach>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="levelSubmit" class="btn btn-primary">
                        点击生成链接
                    </button>
                </div>
            </div><!-- /.modal-content -->
	...
	<script type="text/javascript">
        var baseUrl="<c:url value='/auth/dmz/letterForm.html'/>";
        var letterUrl="";

        //等级Id,
        function level(lid,aid,uid){
            //初始化letterForm
            letterUrl=baseUrl+"?aid="+aid+"&uid="+uid;
                $("#levelModal").modal({
                    backdrop : "static"
                });
        }
		...
		$(function() {
            $("#levelSubmit").click(function(){
                letterUrl+="&lid="+$("#allLevelsSelect").val();
                location.href=letterUrl;
            });
			$(".addBtn").click(
					function(e) {
						e.preventDefault();
