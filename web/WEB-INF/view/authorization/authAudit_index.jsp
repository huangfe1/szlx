				...
				<div class="container-fluid">
					<div class="collapse navbar-collapse" id="top-navbar-collapse">
						<form class="navbar-form navbar-left" id="searchForm"
							role="search" action="<c:url value='/auth/index.html'/>"
							method="GET">
							<div class="form-group">
								<label class="">类型名称</label> <input type="text"
									value="${parameter.entity.authorizationType.name}" name="entity.authorizationType.name" id="name"
									autofocus class="typeahead form-control" placeholder="授权类型名称">
								<%--<label class="">产品名称</label> <input type="text"--%>
									<%--value="${parameter.entity.authorizationType.goods.name}" name="entity.authorizationType.goods.name" id="goodsName"--%>
				...
				<div class="col-lg-12 col-md-12">
					<div class="table-responsive">
						<table
							...
							<tbody id="dataList">
								<c:forEach items="${authorizations}" var="a">
									<tr data-row="${a.id}">
										<td><input type="checkbox" value="${a.id}"></td>
										<td>${a.agent.agentCode}</td>
										<td>${a.agent.realName}</td>
										<td>${a.authorizationType.name}</td>
										<td>${a.updateTime}</td>
										<td><a class="btn btn-success default editBtn"
											href="<c:url value='/auth/edit.html?id=${a.id}' /> "><span class="glyphicon glyphicon-wrench" aria-hidden="true"></span>点击激活</a>
											<%--<a class="ajaxLink btn btn-danger default" data-role="delete"--%>
											<%--href="<c:url value='/auth/remove.json?id=${g.id}' /> "><span class="glyphicon glyphicon-trash" aria-hidden="true"></span>删除</a>--%>
										</td>
									</tr>
								</c:forEach>
							</tbody>
