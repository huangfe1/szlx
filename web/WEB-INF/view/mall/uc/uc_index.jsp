...
<jsp:include page="/WEB-INF/view/mall/mall_menu.jsp"></jsp:include>
	...
	<%-- 	<div class="row">
		...
		<!--我的账户 -->
			...
			<div class="col-xs-12">
				<a class="btn btn-info btn-block btn-lg" href="<c:url value='/agent/myAuth.html?from=${user.id}'/>" id="sw">
					<span class="fa fa-credit-card"></span>&nbsp我的账户
				</a>
			</div>
		</div>
		<br>
        ...
        <div class="row">
            <div class="col-xs-12">
                <a class="btn btn-info btn-block btn-lg" href="<c:url value='/agent/children.html?from=${user.id}'/>" id="sw">
                    <span class="fa fa-sitemap"></span>我的客户
                </a>
            </div>
        </div>
        <br>
		...
		<%--<div class="row">--%>
			<%--<div class="col-xs-12">--%>
				<%--<a class="btn btn-info btn-block btn-lg" href="<c:url value='/advance/pay/index.html'/>" id="sw">--%>
					<%--<span class="glyphicon glyphicon-usd"></span>&nbsp物流费充值--%>
				<%--</a>--%>
			<%--</div>--%>
		<%--</div>--%>
		<%--<br>--%>
		...
		<!-- 向上家进货 -->
			...
			<div class="col-xs-12">
				<a class="btn btn-warning btn-block btn-lg" href="<c:url value='/transfer/my.html?from=${user.id}'/>" id="sw">
					<span class="fa fa-exchange"></span>&nbsp我要转货
				</a>
			</div>
		</div>
		<br>
			...
			<div class="col-xs-12">
				<a class="btn btn-warning btn-block btn-lg" href="<c:url value='/transfer/records.html?from=${user.id}'/>" id="sw">
					<span class="fa fa-list"></span>&nbsp转货纪录
				</a>
			</div>
		</div>
		<br> 
		...
		<!--我的 -->
			<div class="row">
			<div class="col-xs-12">
				<a class="btn btn-success btn-block btn-lg" href="<c:url value='/delivery/index.html'/>" id="sw">
					<span class="fa fa-list"></span>&nbsp发货纪录
				</a>
			</div>
		</div>
		<br>
		...
		<div class="row">
			<div class="col-xs-12">
				<a class="btn btn-primary btn-block btn-lg" href="<c:url value='/voucher/record.html'/>" id="sw">
					<span class="fa fa-dollar"></span>奖金详情
				</a>
			</div>
		</div>
		<br>
		...
		<div class="row">
			<div class="col-xs-12">
				<a class="btn btn-primary btn-block btn-lg" href="<c:url value='/advance/my.html'/>" id="sw">
					<span class="fa fa-cny"></span>转让物流费用
				</a>
			</div>
		</div>
		<br>
		...
		<div class="row">
			<div class="col-xs-12">
				<a class="btn btn-primary btn-block btn-lg" href="<c:url value='/advance/record.html'/>" id="sw">
					<span class="fa fa-cny"></span>物流费用
				</a>
			</div>
		</div>
		<br>
		...
		<div class="row" id="uc">
			<div class="col-md-12 col-xs-12">
				...
				<%--<ul class="list-group">--%>
								<%--&lt;%&ndash;<li class="list-group-item"><span class="badge">${my.accounts.pointsBalance}</span>&ndash;%&gt;--%>
								<%--&lt;%&ndash;积分余额&ndash;%&gt;--%>
								<%--&lt;%&ndash;</li>&ndash;%&gt;--%>
								<%--<li class="list-group-item"><span class="badge">${my.accounts.voucherBalance}</span>--%>
								<%--拥有奖金--%>
								<%--</li>--%>
								<%--&lt;%&ndash;<li class="list-group-item"><span class="badge">${my.accounts.benefitPointsBalance}</span>&ndash;%&gt;--%>
								<%--&lt;%&ndash;拥有福利积分&ndash;%&gt;--%>
								<%--&lt;%&ndash;</li>&ndash;%&gt;--%>
							<%--</ul>--%>
			</div>
		...
		<%--&lt;%&ndash;<c:forEach items="${orders}" var="order">&ndash;%&gt;--%>
						<%--&lt;%&ndash;<div class="panel panel-success">&ndash;%&gt;--%>
							<%--&lt;%&ndash;<div class="panel-heading">订单号:${order.orderNo}&nbsp;&ndash;%&gt;--%>
							<%--&lt;%&ndash;<h4><small>下单时间:<fmt:formatDate type="BOTH" pattern="yyyy年MM月dd日 HH:mm:ss" value="${order.applyTime}"/></small></h4>&ndash;%&gt;--%>
							<%--&lt;%&ndash;<h4><small>物流单号:${order.logisticsCode}</small></h4>&ndash;%&gt;--%>
							<%--&lt;%&ndash;<h4><small>物流公司:${order.logistics}</small></h4></div>&ndash;%&gt;--%>
								<%--&lt;%&ndash;<div class="table-responsive">&ndash;%&gt;--%>
									<%--&lt;%&ndash;<table class="table">&ndash;%&gt;--%>
										...
										<%--&lt;%&ndash;</thead>&ndash;%&gt;--%>
										<%--&lt;%&ndash;<tbody>&ndash;%&gt;--%>
											<%--&lt;%&ndash;<c:forEach items="${order.deliveryItems}" var="o">&ndash;%&gt;--%>
												<%--&lt;%&ndash;<tr>&ndash;%&gt;--%>
													<%--&lt;%&ndash;<td>${o.goods.name}</td>&ndash;%&gt;--%>
													<%--&lt;%&ndash;<td class="text-muted">${o.quantity}</td>&ndash;%&gt;--%>
													<%--&lt;%&ndash;<td>${order.status.desc}</td>													&ndash;%&gt;--%>
												<%--&lt;%&ndash;</tr>&ndash;%&gt;--%>
											<%--&lt;%&ndash;</c:forEach>&ndash;%&gt;--%>
										<%--&lt;%&ndash;</tbody>&ndash;%&gt;--%>
										<%--&lt;%&ndash;&lt;%&ndash; <tfoot>&ndash;%&gt;--%>
											<%--&lt;%&ndash;<tr>&ndash;%&gt;--%>
												<%--&lt;%&ndash;<td colspan="2">${order.status.name}/${order.paymentStatus.name}</td>&ndash;%&gt;--%>
													<%--&lt;%&ndash;<td ><c:if test="${order.status eq 'SHIPPED' }">&ndash;%&gt;--%>
															<%--&lt;%&ndash;<button class="btn btn-success btn-xs receiveBtn" data-url="<c:url value='/pm/order/receive.json?id=${order.id}' />">确认收货</button>&ndash;%&gt;--%>
														<%--&lt;%&ndash;</c:if>&ndash;%&gt;--%>
														<%--&lt;%&ndash;<c:if test="${order.paymentStatus eq 'UNPAID' and order.status eq 'NEW' }">&ndash;%&gt;--%>
															<%--&lt;%&ndash;<button class="btn btn-warning btn-xs"><span class="fa fa-money fa-fw"></span>去支付</button>&ndash;%&gt;--%>
														<%--&lt;%&ndash;</c:if>&ndash;%&gt;--%>
														<%--&lt;%&ndash;</td>&ndash;%&gt;--%>
											<%--&lt;%&ndash;</tr>&ndash;%&gt;--%>
										<%--&lt;%&ndash;</tfoot> &ndash;%&gt;&ndash;%&gt;--%>
									<%--&lt;%&ndash;</table>&ndash;%&gt;--%>
								<%--&lt;%&ndash;</div>&ndash;%&gt;--%>
								<%--&lt;%&ndash;<c:if test="${order.status eq 'NEW' }">&ndash;%&gt;--%>
								<%--&lt;%&ndash;<div class="panel-footer"><button class="btn btn-warning btn-xs revokeBtn"&ndash;%&gt;--%>
								 <%--&lt;%&ndash;data-id="${order.id}">撤销订单</button></div></c:if>&ndash;%&gt;--%>
						<%--&lt;%&ndash;</div>&ndash;%&gt;--%>
					<%--&lt;%&ndash;</c:forEach>&ndash;%&gt;--%>
		<%--</div>--%>
		<%--</div>--%>
