...
public class DeliveryNoteQueryController {
	...
	public String index(
		...
		try {
			...
			//仓库描述一样的
            List<Logistics> logisticses=logisticsDAO.findAll();
            Map<String,Logistics> tempLog=new HashedMap();
            for(Logistics log : logisticses){
                if(!tempLog.containsKey(log.getWeights())){
                    tempLog.put(log.getWeights(),log);
                }
            }
            model.addAttribute("logisticses", tempLog.keySet());
		} catch (Exception exp) {
			exp.printStackTrace();
    ...
    public String edit(
        ...
        List<Logistics> logisticses=logisticsDAO.findAll();
        model.addAttribute("ls",logisticses);
	...
	public void download(
        ...
        Map<String,Integer> sumMap=new HashedMap();//纪录总数
...
//        headers.add("产品数量");
		...
		for(int i=0;i<orders.size();i++){
			...
			for(DeliveryItem item:order.getDeliveryItems()){//遍历所有的item
                ...
                if(sumMap.containsKey(gn)){
                    sumMap.put(gn,sumMap.get(gn)+gq);
                }else {
                    sumMap.put(gn,gq);
                }
        ...
        for(String key:sumMap.keySet()){
            m1=new HashedMap();
            m1.put(0,key);
            m1.put(1,sumMap.get(key));
            datas1.add(m1);
        }
        List<String> sheetses=new ArrayList<>();
        sheetses.add("商城订单");
        sheetses.add("货物总数");
        List<List> headerses=new ArrayList<>();
        headerses.add(headers);
        headerses.add(headers1);
        List<List<Map>> datases=new ArrayList<>();
        datases.add(datas);
        datases.add(datas1);
	    ExcelFile.ExpExs("d",sheetses,headerses,datases,response);
    }
