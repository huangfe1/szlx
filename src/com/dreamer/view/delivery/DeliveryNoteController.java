	...
	public Message edit(
		...
		try {
			if((Objects.isNull(agentCode)||agentCode.isEmpty()) && (Objects.isNull(parameter.getConsigneeName())||parameter.getConsigneeName().isEmpty())){
				throw new ParameterMissingException("请选择地址!");
			}
			...
			if(parameter.getVersion()==null){
				...
				parameter.setUserByConsignee(agent);
...
//                String company="百事快递";
	...
	public Message delivery(
			@ModelAttribute("parameter") DeliveryNote parameter,
			HttpServletRequest request, Model model,Double actual_logisticsFee,Integer sfgzId) {
		try {
			User user=(User)WebUtil.getCurrentUser(request);
			if(!user.isAdmin()){
				throw new DataAccessException("非管理员无发货权限");
			}
			Logistics logistics=logisticsDAO.findById(sfgzId);
			parameter.setSfgz(logistics);
