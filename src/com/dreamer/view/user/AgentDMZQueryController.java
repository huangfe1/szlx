...
public class AgentDMZQueryController {
		...
		try {
			...
			if(Objects.nonNull(agent)){
				...
				if(agent.hasMainGoodsAuthorization(null)){
						...
						return au.getAuthorizedGoods().isMainGoods()&&au.isActive()&&au.isActive();
					}).collect(Collectors.toList());
                }else{//没有主打产品查询所有激活了的产品
					auths=agent.getAuthorizations().stream().filter(au->{return  au.isActive();}).collect(Collectors.toList());
				}
	...
	public void qrCodeForDomain(
		...
		try {
			byte[] bts = QRCODE.generateAsBytes(WebUtil.getContextPath(request)
					+ parameter.getEntity().getSubDomain(), 200, 200, 0,
			...
			gbg.drawImage(tag, 200, 490, null);
...
//			gbg.drawString(parameter.getEntity().getRealName(), 290-parameter.getEntity().getRealName().length()*50, 145);
			...
			gbg.drawString(parameter.getEntity().getRealName(), 305-parameter.getEntity().getRealName().length()*10, 432);
			gbg.drawString(parameter.getEntity().getAgentCode(), 250, 468);
