...
public class AgentQueryController {
	...
	public String agentAuthorizations(
		...
		List<GoodsAccountDTO> gacs=parameter.getEntity().getGoodsAccounts().stream().map(gac->{
			...
			dto.setCurrentCumulative(gac.getCurrentCumulative());
			dto.setCumulativeRecord(gac.getCumulativeRecord());
        ...
        //add by hf  找出所有的level
        List<AgentLevel> allLevels =agentLevelDAO.findAll();
