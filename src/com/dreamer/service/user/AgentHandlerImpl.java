...
public class AgentHandlerImpl implements AgentEventSource, AgentHandler{
	...
	public Agent selfRegister(Agent agent,String referrerAgentCode){
		...
		//this.fireEvent(new AgentEvent(agent));  注册触发时间
		agentDAO.save(agent);
		return agent;
	}
     ...
     */
    @Transactional
    public Agent addAuthAndGac(Agent agent,Integer aid,Integer lid){
        System.out.println("正在增加授权");
        AuthorizationType authorizationType= authorizationTypeDAO.findById(aid);
        agent.addAuthorization(authorizationType);//增加一个授权
        AgentLevel agentLevel=agentLevelDAO.findById(lid);//
        agent.addGoodsAccount(authorizationType.getGoods(),agentLevel);//添加等级账户
        agentDAO.save(agent);
        return agent;
    }
...
//	 * 授权申请  哪个用户的,哪个授权
//	 */
//	private void applyAuth(Agent agent,Authorization authorization){
//
//	}
	...
	public void transferAdvance(User operator, AdvanceTransfer transfer,
		...
		transfer.transferAdvance(fromAgent, toAgent, transferVoucher);
