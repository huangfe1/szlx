...
//		Logistics logistics = logisticsDAO.findByProvinces(note.getProvince());
		Logistics logistics = logisticsDAO.findById(note.getSfgz().getId());
		if(isAdd){//如果要新增加地址
		...
		if(!note.isMallApply()){
		note.getUserByApplyAgent().getAccounts().deductAdvance(note.getLogisticsFee(),"发货订单"+note.getOrderNo()+"扣减物流费用"+note.getLogisticsFee());
		}
	...
	public void buildDeliveryNoteByTmallAndConfirm(DeliveryNote note,
		...
		agent.getAccounts().deductAdvance(note.getLogisticsFee(),"店铺直接发货"+note.getOrderNo()+"扣减物流费");
		deliveryNoteDAO.save(note);
		cart.getItems().clear();
	}
...
//			}
	...
	public void removeDeliveryNote(DeliveryNote note) {
		...
		if (!note.isMallApply()) {//店铺下单的时候 并没有扣除物流费
			note.getUserByApplyAgent().getAccounts().increaseAdvance(note.getLogisticsFee(),"发货订单"+note.getOrderNo()+"物流费退回");
		}
	...
	public void confirm(DeliveryNote note) {
		...
		if(!parent.isMutedUser())//上级不是公司,扣减物流
			parent.getAccounts().deductAdvance(note.getLogisticsFee(),"确定下级发货订单"+note.getOrderNo()+"扣减物流费");
		deliveryNoteDAO.merge(note);
	}
...
//		}
	...
	public void deliveryNoteAutoTransferToApplyAgent(DeliveryNote note,Boolean isUseVoucher) {
		...
		transfer.setAdvance(advance_temp);//扣减可用的物流费
		//先扣减物流费
	...
	public void delivery(DeliveryNote note,Double actual_logisticsFee) {
		...
		if(actual_logisticsFee<note.getLogisticsFee()){
			...
			note.getUserByApplyAgent().getAccounts().increaseAdvance(temp,"物流费退回,订单"+note.getOrderNo());
