...
public class GoodsAccountDAO extends HibernateBaseDAO<GoodsAccount>{
     ...
     * 将本月的进货量清零
     */
	public void  retSetByMonth(){
		String sql="update goods_account set cumulativeRecord=concat(cumulativeRecord,'_',currentCumulative),currentCumulative=0";
        try {
            getCurrentSession().createSQLQuery(sql).executeUpdate();//执行
