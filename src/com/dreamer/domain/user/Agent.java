...
//		GoodsAccount account=getGoodsAccounts().stream().filter(g->g.getGoods().getGoodsType().equals(GoodsType.TEQ)&&(g.getAgentLevel().getName().contains(name1)||g.getAgentLevel().getName().contains(name2))).findFirst().orElse(null);
//		return !Objects.isNull(account);
	...
	public boolean isVip(){
		GoodsAccount ga=mainGoodsAccount(null).get(0);
		if(ga!=null&&ga.getAgentLevel().getName().equals(AgentLevelName.全国总代.toString())){
		return true;
		}
		return  false;
	}
	...
	public boolean isGd(){
		GoodsAccount ga=mainGoodsAccount(null).get(0);
		if(ga!=null&&ga.getAgentLevel().getName().equals(AgentLevelName.股东.toString())){
			return true;
		}
		return  false;
	}
    ...
    /**
     * 只要主打产品是全国总代 且库存累计超过200  就是Flvip  可以接受返利
     * @return true or false
     */
    public boolean isFlVip(){
        int sum=getGoodsAccounts().stream().mapToInt(q-> q.getCurrentCumulative()).sum();
        if(isVip()&&sum>=200){
            return true;
        }
        return  false;
...
//		GoodsAccount account=getGoodsAccounts().stream().filter(g->g.getGoods().getGoodsType().equals(GoodsType.MALL)&&(g.getAgentLevel().getName().contains(name1))).findFirst().orElse(null);
//		return !Objects.isNull(account);
//	}
	...
	public Accounts generateAccounts() {
		...
		accounts.setAdvanceBalance(0d);//设置物流费
     ...
     * @return
     */
    public GoodsAccount addGoodsAccount(Goods goods,AgentLevel agentLevel) {
        if (!hasGoodsAccount(goods)) {
            GoodsAccount account = buildGoodsAccount(goods,agentLevel);
            addGoodsAccount(account);
            return account;
        }
        return null;
     ...
     * @return
     */
    public GoodsAccount buildGoodsAccount(Goods g,AgentLevel agentLevel) {
        //找出主打产品等级
		...
		account.setCumulativeRecord("");
		account.setCurrentCumulative(0);
        account.setCumulative(0);
        account.setAgentLevel(agentLevel);//添加我设置的等级
	...
	public GoodsAccount buildGoodsAccount(Goods g) {//新增账户、有主打产品的MALL新增主打产品等级  没有的最低等级
		...
		account.setCurrentPoint(0D);
		account.setCumulative(0);
		account.setAgentLevel(mainLevel==null?g.getLowestPrice().getAgentLevel():mainLevel);//添加最低等级
		account.setUpdateTime(new Date());
		return account;
	}
	/**
	...
	public void addAuthorization(AuthorizationType authorizationType) {
		Authorization authorization =buildAuthorization(authorizationType);
		if (authorizations.add(authorization)) {
			authorization.setStatus(AgentStatus.NO_ACTIVE);
			authorization.setAgent(this);
		}
	}
	...
	public static void main(String[] args) {
		String idCard = "430102198202235559";
